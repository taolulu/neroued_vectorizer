...
bool MakeStrokeShape(const std::vector<Vec2f>& path, const cv::Mat& dist_map, const Rgb& color,
                     VectorizedShape& out) {
    ...
    float avg_width = width_samples > 0 ? total_width / static_cast<float>(width_samples) : 1.0f;
...
std::vector<VectorizedShape> ExtractStrokePaths(const cv::Mat& skeleton, const cv::Mat& dist_map,
    ...
    for (const auto& ep : endpoints) {
        ...
        if (MakeStrokeShape(path, dist_map, color, shape)) shapes.push_back(std::move(shape));
    }
